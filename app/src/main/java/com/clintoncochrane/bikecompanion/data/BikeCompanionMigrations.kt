...
object BikeCompanionMigrations {
    ...
    val MIGRATION_9_10 = object : Migration(9, 10) {
        override fun migrate(db: SupportSQLiteDatabase) {
            addColumnIfNotExists(db, "rides", "hadPlaceholdersAtStart", "INTEGER NOT NULL DEFAULT 0")
        }
    }

    val MIGRATION_10_11 = object : Migration(10, 11) {
        override fun migrate(db: SupportSQLiteDatabase) {
            addColumnIfNotExists(db, "bikes", "baselineDistanceKm", "REAL NOT NULL DEFAULT 0")
        }
    }

    /**
     * Existing components default to UNKNOWN because their tracked distance does not prove
     * whether any usage occurred before Bike Companion began tracking them.
     */
    ...
    /** Stores the provider-assigned identity used to prevent duplicate Health Connect imports. */
    val MIGRATION_13_14 = object : Migration(13, 14) {
        override fun migrate(db: SupportSQLiteDatabase) {
            addColumnIfNotExists(db, "rides", "healthConnectRecordId", "TEXT")
            db.execSQL(
                "CREATE UNIQUE INDEX IF NOT EXISTS index_rides_healthConnectRecordId " +
                    "ON rides(healthConnectRecordId)",
            )
        }
    }
    ...
    /** Keeps the latest completed-service boundary for interval reconciliation. */
    val MIGRATION_16_17 = object : Migration(16, 17) {
        override fun migrate(db: SupportSQLiteDatabase) {
            addColumnIfNotExists(db, "service_intervals", "lastCompletedAt", "INTEGER")
        }
    }
